...
const MultiSelectorTrigger = forwardRef<
  HTMLDivElement,
  React.HTMLAttributes<HTMLDivElement> & {shops?: Shop[]}
>(({ className, shops, children, ...props }, ref) => {
  ...
  return (
    <div
      ref={ref}
      className={cn(
        "flex flex-wrap gap-1 p-1 py-1 ring-1 ring-muted rounded-lg bg-background",
        {
    ...
    >
      {value.map((item, index) => (
        <Badge
          ...
          variant={"secondary"}
        >
          <span className="text-xs">{shops ? shops.find(shop => shop.id == item)?.name : item}</span>
          <button
            aria-label={`Remove ${shops ? shops.find(shop => shop.id == item)?.name : item} option`}
            aria-roledescription="button to remove option"
            type="button"
            onMouseDown={mousePreventDefault}
            onClick={() => onValueChange(item)}
          >
            <span className="sr-only">Remove {shops ? shops.find(shop => shop.id == item)?.name : item} option</span>
